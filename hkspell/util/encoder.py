...
class LabelEncoder:
    ...
    def __init__(self, character_set: list, pad_index = 0, max_len: int = None, add_padding = False) -> None:
    ...
    def encode_text(self, text: str):
        ...
        return encoded + [self.pad_index] * (self.max_len - len(encoded)) if self.add_padding and self.max_len > len(encoded) \
            else encoded

    def encode(self, texts):
        return [self.encode_text(text) for text in texts] if type(texts) is list \
...
class EnglishCharacterEncoder(LabelEncoder):
    ...
    def __init__(self, max_len: int = None, add_padding = False) -> None:
        super().__init__(" abcdefghijklmnopqrstuvwxyz", max_len=max_len, add_padding=add_padding)
